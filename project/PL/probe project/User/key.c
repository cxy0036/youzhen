...
void Key_Process(void)
{
	...
	{
		Key_Scan();//
		if(((!((~P1)&0x01))&&(Full_Charge_Flag)&&(Skin_Tpye == 5))||((Skin_Tpye == 3)&&(Flash_Level == 9))||((Skin_Tpye == 4)&&(Flash_Level == 9)))//在满电下有触摸，颜色感应不为1/0时闪烁
			{
					Skin_Tpye5_Count++;
      ...
      if((~Key_Count) & TOUCH_PIN)	
		...
		if(Full_Charge_Flag && (!(Error_Flag)) && (!(Low_Lamp)) && (!(Skin_Tpye == 0))&& (!(Skin_Tpye == 5)))
			//if(Full_Charge_Flag && (!(Error_Flag)) && (!(Low_Lamp)))
		{		
			switch(Mode_Type){
			//case Auto_Mode : Auto_Flash() ;break;
			case Auto_Mode : Auto_Com(1) ;break;
			case Auto_Multi_Mode :Auto_Com(4);break;
...
void Auto_Com(uint8_t plustimes)
{
