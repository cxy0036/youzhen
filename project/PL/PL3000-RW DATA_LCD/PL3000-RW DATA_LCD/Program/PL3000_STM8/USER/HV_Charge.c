...
{  
  ...
  if(plus_times > 1)
  {
    ...
    //}else{
    //  flash_over_thr = HV_PID.SetValue-2;
    //}   
  }else{
    multi_flag = 0;
 ...
 /* switch(HV_Level){
  case 1: Flash_Interval_Time = 0;break;
  case 2: Flash_Interval_Time = 57;break;
  case 3: Flash_Interval_Time = 114;break;
  case 4: Flash_Interval_Time = 171;break;
  case 5: Flash_Interval_Time = 228;break;
  case 6: Flash_Interval_Time = 285;break;
  case 7: Flash_Interval_Time = 342;break;
  case 8: Flash_Interval_Time = 399;break;
  default :break;
  }*/
  ...
  while(!Flash_Start)  /* wait flash start */
  {
    ...
    if(Flash_Time_Out == 300)
    {
      //Flash_ERROR = 1;//error; 
      SET_IO_LOW(PROBE_POWER_PORT,PROBE_POWER_PIN);
      Delay_ms(30);
      SET_IO_HIGH(PROBE_POWER_PORT,PROBE_POWER_PIN); 
  ...
  while(plus_times)
  {
    ...
    {     
      IWDG->KR = 0xAA;
      switch(HV_Level){
      case 1:Multi_Charge_Control(100,00);break;  //100ms
      case 2:Multi_Charge_Control(100,20);break;  //120ms
      case 3:Multi_Charge_Control(130,20);;break;  //150ms
      case 4:Multi_Charge_Control(150,30);;break;  //200ms
      case 5:Multi_Charge_Control(200,40);;break;  //260ms
      case 6:Multi_Charge_Control(250,50);;break;  //330ms
      case 7:Multi_Charge_Control(300,60);;break;  //410ms
      case 8:Multi_Charge_Control(340,70);;break;  //500ms
      default:break;
      }     
      ...
      read_hv = ADC_Read_Channel_Fun(ADC_HV_IN_CHANNEL); 
      Flash_Time_Out = 0;
      while(read_hv < HV_PID.SetValue-24)
      {
        Flash_Time_Out++;
        read_hv = ADC_Read_Channel_Fun(ADC_HV_IN_CHANNEL);
        IWDG->KR = 0xAA;
      ...
      SET_IO_HIGH(HV_CONTROL_PORT,HV_CONTROL_PIN);  // OFF HV, charge over,to start next flash     
      if(Flash_Interval_Time > (Flash_Time_Out/1000)*300)
      {
        Delay_ms(Flash_Interval_Time - (Flash_Time_Out/1000)*300);//
      }*/
    }
    if(plus_times==0)
    {     
    ...
    Touch_Key_Not_Release = Receive_Wait_Lost_First(5000);
...
void Full_Charge_Judgment(uint16_t read_hv)
{
  ...
  if((Mode_Flag == AUTO_MULTI_MODE) || (Mode_Flag == AUTO_MODE))
  {
    Charge_Wait_Time = 250;
  }else{
       Charge_Wait_Time=123; 
    }
  
  if((Charge_Time_Count >= Charge_Wait_Time) || (Full_Charge_Flag))//
  {
    if((read_hv <= HV_PID.SetValue+5) && (read_hv >= HV_PID.SetValue-5))
    {     
      if(!Full_Charge_Flag)  /* befor not full charge */
      {
        ...
        if(Judgment_Times == 14)  /* delay some time set full charge flag */
        {
          Full_Charge_Flag = 1;
          Full_Charge_Beep_Flag = 0;
          Discharge_Flag = 0;
